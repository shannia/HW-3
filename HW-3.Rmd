...
* Roughly how much time you spent on this HW so far: 2.5 hours
...
testing_set <- anti_join(profiles, training_set)
...
bitcoin <- Quandl("BAVERAGE/USD") %>% 
  ...
  mutate(bvolatility = (`24h Average`-prev_day_value)/prev_day_value) %>% 
  filter(!is.na(bvolatility))

gold <- Quandl("BUNDESBANK/BBK01_WT5511") %>%  
  mutate(prev_day_value = lead(Value, k = 1)) %>% 
  mutate(gvolatility = (Value-prev_day_value)/prev_day_value) %>% 
  filter(!is.na(gvolatility))

bg <- inner_join(bitcoin, gold, by="Date") %>% 
  select(Date, bvolatility, gvolatility)

bg_volatility <- ggplot() + geom_line(data = bg, aes(x=Date, y=bvolatility), color = "blue") +
  geom_line(data = bg, aes(x=Date, y=gvolatility), color = "red")
bg_volatility +
  labs(title="Volatility of Gold and Bitcoin", x="Volatility by percentage", y="Date by Year")

```


Gold is much more volatile on a day to day basis compared to bitcoin. My non-econ-educated advice would be to invest in bitcoin to be safe, unless you're very certain about the value growth of gold (insider trading?).
